...
router.post('/edit-recipe', function(req, res) {
  if (req.body.action === "Save Recipe") {
    ...
    async.waterfall([
      ...
      if (err) {
        logger.error("Unable to edit recipe for {userId = %s, recipeId = %s, categoryId = %s, directions = %s, ingredients = %s, name = %s}", req.user.userId, req.body.recipeId, categoryId, directions, ingredients, recipeName);
        logger.error(err);
